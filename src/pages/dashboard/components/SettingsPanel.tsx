...
export function SettingsPanel({ settings, onSave }: Props) {
  ...
  async function handleTest() {
    ...
    try {
      ...
      setTestResult(`Success — received ${JSON.stringify(data).slice(0, 120)}…`)
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl space-y-8">
      ...
      <section>
        ...
        <div className="space-y-4">
          ...
          <Field
            ...
            hint='Dot-notation path to the array in the response. E.g. "data" or "results.items". Leave blank if the root is the array.'
          >
            <input
              type="text"
              value={form.dataPath}
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            {testResult && (
              <span className={`text-xs break-all ${testResult.startsWith('Success') ? 'text-green-600' : 'text-red-600'}`}>
                {testResult}
              </span>
            )}
          </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field
            label={`Concurrency: ${form.concurrency}`}
            hint="Number of URLs checked simultaneously"
          >
            <input
              type="range"
              min={1} max={200}
          ...
          <Field
            label={`Timeout: ${form.timeoutMs / 1000}s`}
            hint="Per-URL request timeout before marking as Timeout"
          >
            <input
              type="range"
